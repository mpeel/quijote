...
for k in range(0,len(prefixes)):
	...
	for i in range(0,numres):
		...
		smoothnoisemap(directory, 'TEST'+str(output_nside[i])+'_'+resolution+'smoothed_'+newprefixes[k]+'2_17.0_512_'+newdate+'_nhits', prefixes[k]+'_nhits_17.0_2.fits',mapnumber=0,numrealisations=numrealisations,fwhm=output_resolution[i],sigma_0=1.0*np.sqrt(25),nside=output_nside,windowfunction=np.sqrt(mfi_217),usehealpixfits=True)

		smoothnoisemap(directory, str(output_nside[i])+'_'+resolution+'smoothed_'+newprefixes[k]+'2_17.0_512_'+newdate+'_weight', prefixes[k]+'_weights_17.0_2.fits',mapnumber=0,numrealisations=numrealisations,fwhm=output_resolution[i],nside=output_nside,windowfunction=np.sqrt(mfi_217),usehealpixfits=True)
		exit()
